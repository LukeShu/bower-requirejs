...
module.exports = function (name, dep, opts) {
  ...
  function findByPackage() {
    ...
    try {
      ...
      throw new Error('Unable to parse "' + pkgPath + '" file (' + err.message + ').', err);
    }
    if (pkg.main) {
      main = [pkg.main];
    }
  ...
  _.each(searchDirs, function (searchDir) {
    ...
    var candidateFiles = _.filter(
      glob.sync("*.js", { cwd: normalizePath(searchPath)}),
      function(fname) { return !fname.endsWith(".min.js"); });
